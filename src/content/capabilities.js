/**
 * Core Capabilities — edit titles, descriptions, and items here.
 */
export const capabilitiesSection = {
  eyebrow: "Expertise",
  title: "Core Capabilities",
  intro:
    "The strengths I bring to engineering, product, and customer-facing teams — from architecture to the people who use the software.",
};

export const capabilities = [
  {
    icon: "layers",
    title: "Full-Stack Engineering",
    skillsColumns: 3,
    description:
      "End-to-end product development — from data models and APIs to interfaces teams rely on every day.",
    items: [
      "Python",
      "Django",
      "React",
      "R.APIs",
      "MySQL",
      "SQLite",
      "Flask",
      "C#",
      "JS",
      "Git",
      "GitHub",
      "HTML5",
      "B.strap",
      "CSS3",
      "AWS",
    ],
  },
  {
    icon: "sparkles",
    title: "AI & Modern Development",
    skillsColumns: 2,
    skillsTextCenter: true,
    description:
      "Designing intelligent features and shipping faster with disciplined, AI-assisted engineering workflows.",
    items: [
      "LLM Applications",
      "LLM Workflows",
      "Prompt Engineering",
      "AI-Assisted Development",
      "API Integration",
      " Voice Interfaces ",
      
    ],
  },
  {
    icon: "users",
    title: "Customer & Product",
    skillsColumns: 2,
    skillsTextCenter: true,
    description:
      "Understanding how software performs for users, support teams, and the business behind the product.",
    items: [
      "Customer Support",
      "Customer Success",
      "Technical Support",
      "Product Thinking",
      "Functional Collaboration",
      "Issue Resolution",

    ],
  },
  {
    icon: "shieldCheck",
    title: "Engineering Practices",
    skillsColumns: 2,
    description:
      "Clean architecture, reliable delivery, and standards that keep products maintainable as they grow.",
    items: [
      "API Design",
      "Database Design",
      "Software Documentation",
      "Testing & Quality Assurance",
      "Code Review",
      "Responsive Design",
    ],
  },
  {
    icon: "lightbulb",
    title: "Problem Solving",
    skillsColumns: 2,
    description:
      "Breaking down complex technical and operational issues into clear, actionable solutions.",
    items: [
      "Troubleshooting",
      "Business Analysis",
      "Root Cause Analysis",
      "Performance Optimization",
    ],
  },
  {
    icon: "workflow",
    title: "Workflow Optimization",
    skillsColumns: 2,
    description:
      "Improving how teams work — streamlining processes, documentation, and the path from idea to production.",
    items: [
      "Process Improvement",
      "Documentation",
      "Git & GitHub",
      "Delivery Pipelines",
    ],
  },
];
